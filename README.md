...
| Flag | Description |
----------------------
| -p | Profile to install (required) |
| -i | Install Debian packages for the profile |
| -f | Force overwrite existing config files |
| -s | List stow packages in a profile |
| -b | List Debian packages in a profile |
...
| File | Purpose |
------------------
| stow.pkglist | One stow package name per line |
| debian.pkglist | One apt package name per line |
| profile.deps | One profile name per line (dependencies) |
| build.sh | Executable script for building from source |
